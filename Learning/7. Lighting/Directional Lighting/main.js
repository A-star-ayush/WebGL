// There are many ways to implement lighting. Perhaphs the simplest is "directional lighting".
// Directional lighting assumes the light is coming uniformly from one direction. The sun on a clear day is 
// often considered a directional light. It's so far way that its rays can be considered to be hitting the surface 
// of an object all in parallel.

/* If we know what direction the surface of our 3d object is facing and we know the direction the light is shining 
then we can just take the dot product of them and it will give us a  number 1 if the light is pointing directly at 
the surface and -1 if they are pointing directly opposite. We can multiply our color by that dot product value and 
boom! Light!*/

/* So, let's go add normals to our F from our previous examples so we can light it. Since the F is very boxy and 
   its faces are aligned to the x, y, or z axis it will be pretty easy. Things that are facing forward have the 
   normal 0, 0, 1. Things that are facing away are 0, 0, -1.  Facing left is -1, 0, 0, Facing right is 1, 0, 0. 
   Up is 0, 1, 0 and down is 0, -1, 0. */

// For the reason behind transposing the matrix for normal calculations refer to the slides from the edx course
...
function start(){
	...
	var u_reverseLD_loc = gl.getUniformLocation(program, "u_reverseLD");
	...
	gl.uniform3fv(u_reverseLD_loc, normalize([0.5, 0.7, 1]));
